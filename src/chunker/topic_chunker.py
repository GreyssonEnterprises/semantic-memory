...
2. Sliding Window: Overlapping windows with topic-transition prefixes.
   Best for ADHD-brain sessions (Dean/Jackie) where topic shifts are fast and
...
def load_domain_taxonomy(taxonomy_path: Optional[str] = None) -> Optional[dict]:
    ...
    # NAS shared location
    search_paths.append(
        Path("/Volumes/public/projects/business/warehouse/"
             "semantic-memory-cross-agent/data/domain-taxonomy.json")
    )
